...
def same_first_last(nums):
  if len(nums) > 0 and (nums[0] == nums[-1] or  nums[-1] == nums[0]):
    return  True
  else:
    return False
...
def common_end(a, b):
  if a[0] == b[0] :
    return True
  elif a[:-2:-1]== b[:-2:-1]: #This shouldn't work for this exercise but it works.
    return True
  else:
    return False 
...
def sum2(nums):
  return sum(nums[:2])#I didn't know about the sum function
...
def squirrel_play(temp, is_summer):
  if (90 >= temp >= 60 and is_summer==False) or (60 <= temp <= 100 and is_summer==True):
    return True
...
def pego_correndo(speed, is_birthday):
  if (is_birthday ==True  and speed <= 65) or speed <=60:
    return 0
  elif 80 >= speed >= 61 or is_birthday == True and speed < 86:
    return 1
  elif speed > 81:
    return 2
# J. alarm_clock #
