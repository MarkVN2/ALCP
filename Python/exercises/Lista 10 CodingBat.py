...
# lone_sum(3, 3, 3) -> 0
def lone_sum(a, b, c):
  #This is going to look bad since i am going to do some bad code since i am lazy.
  if a == b == c :
    return 0
  if a == b :
    return c 
...
def lucky_sum(a, b, c):
  if a == 13 :
    return 0 #Because A is the first number in the list
  if b == 13 :
    return a 
  if c == 13 :
    d = a + b 
...
def double_char(s):
  return ''.join(l + l for l in s) #for ever letter in s add another one.
...
def count_code(s):
  ct = 0
  for d in range(0,len(s)-3):
    if s[d:d+2] == 'co' and s[d+3] == 'e': #Verifies if two the next two letters are co and the fourth E
      ct += 1
  return ct
...
def end_other(a, b):
  a = a.lower()
  b = b.lower()
  if b.endswith(a) or a.endswith(b) : #didn't know about endswith fuction (discovered after researching)
    return True
  else:
    return False
...
def fila_tijolos(n_peq, n_gra, meta):
  return n_peq + n_gra*5 >= meta and n_peq >= meta % 5 #Last part of this is confusing
